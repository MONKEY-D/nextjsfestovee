...
const Filter = () => {
  ...
  const handleCategoryFilter = (categorySlug) => {
    const newSelectedCategory = selectedCategory.includes(categorySlug)
      ? selectedCategory.filter((cat) => cat !== categorySlug)
    ...
    if (newSelectedCategory.length > 0)
      urlSearchParams.set("category", newSelectedCategory.join(","));
    else urlSearchParams.delete("category");
  ...
  const handleColorFilter = (color) => {
    ...
    if (newSelectedColor.length > 0)
      urlSearchParams.set("color", newSelectedColor.join(","));
    else urlSearchParams.delete("color");
  ...
  const handleSizeFilter = (size) => {
    ...
    if (newSelectedSize.length > 0)
      urlSearchParams.set("size", newSelectedSize.join(","));
    else urlSearchParams.delete("size");
  ...
  return (
    <div>
      {/* Clear Filter button only if any filter applied */}
      ...
      <Accordion type="multiple" defaultValue={["1", "2", "3", "4"]}>
        ...
        <AccordionItem value="2">
          ...
          <AccordionContent>
            <div className="max-h-48 overflow-auto">
              <ul>
                {colorData?.success && Array.isArray(colorData.data) ? (
                  colorData.data.map((color) => (
                    <li key={color} className="mb-3">
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <Checkbox
        ...
        <AccordionItem value="3">
          ...
          <AccordionContent>
            <div className="max-h-48 overflow-auto">
              <ul>
                {sizeData?.success && Array.isArray(sizeData.data) ? (
                  sizeData.data.map((size) => (
                    <li key={size} className="mb-3">
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <Checkbox
